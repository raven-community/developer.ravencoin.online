...
#### Examples
      ...
      RVNBOX.ECPair.fromPublicKey(pubkeyBuffer);
...
#### Examples
      ...
      let ecpair = RVNBOX.ECPair.fromPublicKey(Buffer.from('02d305772e0873fba6c1c7ff353ce374233316eb5820acd7ff3d7d9b82d514126b', 'hex'));
      // create public key buffer
      RVNBOX.ECPair.toPublicKey(ecpair);
      //

      // create ecpair from testnet public key buffer
      let ecpair = RVNBOX.ECPair.fromPublicKey(Buffer.from('024a6d0737a23c472d078d78c1cbc3c2bbf8767b48e72684ff03a911b463da7fa6', 'hex'));
      // create public key buffer
      RVNBOX.ECPair.toPublicKey(ecpair);
      //
...
#### Examples
      ...
      let ecpair = RVNBOX.ECPair.fromWIF(wif);
      // to legacy address
      RVNBOX.ECPair.toLegacyAddress(ecpair);
...
#### Examples
      ...
      // ecpairs from wifs
      let ecpair1 = RVNBOX.ECPair.fromWIF(wif1);
      let ecpair2 = RVNBOX.ECPair.fromWIF(wif2);
      // 32 byte buffer
      let buf = Buffer.from(RVNBOX.Crypto.sha256('EARTH'), 'hex');
      // sign
      let signature = RVNBOX.ECPair.sign(ecpair1, buf);
      // verify
      RVNBOX.ECPair.verify(ecpair1, buf, signature);
      // true
      RVNBOX.ECPair.verify(ecpair2, buf, signature);
      // false
