...
exports.onCreateNode = ({ node, getNode, actions }) => {
  ...
  if (node.internal.type === `MarkdownRemark`) {
    ...
    const isChapter = filePath.includes('/mastering-ravencoin/')
    let slug = filePath

    if (isDoc) {
      let product = 'other'

      const isRvnbox = filePath.includes('/rvnbox/')
      const isGui = filePath.includes('/gui/')
      const isRest = filePath.includes('/rest/')

      if (isRvnbox) {
        slug = `/rvnbox/docs/${filename}`
        product = 'rvnbox'
      }
      if (isGui) {
...
exports.createPages = async ({ graphql, actions }) => {
  ...
  // Generate mastering-ravencoin pages
